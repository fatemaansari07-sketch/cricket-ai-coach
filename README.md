...
## Gemini video analysis (Pro tier)
1. Get a free key at https://aistudio.google.com/app/apikey
2. Add `GEMINI_API_KEY` to Vercel env vars (server-only — no `VITE_` prefix, or it leaks to the browser)
3. On the Analyze screen, Pro users see a "Gemini AI se analyze karo" toggle. When on, the video (already uploaded to Storage) is sent server-side (`api/analyze-video-gemini.js`) to Gemini 1.5 Flash for a full-video, any-camera-angle read — instead of the single-frame MediaPipe check.
4. This costs real API usage once you're past Gemini's free tier — that's why it's gated to Pro and never called directly from the browser (the key would be stolen instantly if it were).
5. The prompt deliberately does NOT tell Gemini to favor "famous players" — it grades only what it can see, and says so honestly when the camera angle makes something hard to judge.
