 ...
 *
 * Everything below lives inside ONE try/catch, on purpose: if any step
 * throws (auth, DB, download, provider), the client still gets back JSON
 * with a real error message instead of Vercel's raw crash page, which the
 * frontend can't JSON.parse.
 */
...
`.trim();
    ...
    // Best-effort cache write — if this fails, don't fail the whole request,
    ...
    return res.status(500).json({ error: err?.message || "Analysis failed, try again in a moment." });
  }
}
