 ...
 */
export default function SkeletonOverlay({ landmarks, jointStatus, jointDetail, handedness = "right", neutralColor: neutralColorProp }) {
  ...
  const statusOf = (key) => {
    if (!(key in jointStatus)) return "neutral"; // not tracked in this phase — don't imply it's fine
    return jointStatus[key] === false ? "bad" : "ok";
  };
  const colorOf = (key) => {
    const s = statusOf(key);
    return s === "neutral" ? neutralColor : s === "bad" ? badColor : okColor;
  };

  const okColor = C.green;
  const badColor = C.red;
  const neutralColor = neutralColorProp || "#5B6472";

  const headColor = colorOf("headTiltDeg");
  const elbowColor = colorOf("frontElbowDeg");
  const kneeColor = colorOf("frontKneeDeg");
  const headOk = statusOf("headTiltDeg") !== "bad";
  const elbowOk = statusOf("frontElbowDeg") !== "bad";
  const kneeOk = statusOf("frontKneeDeg") !== "bad";
  ...
  return (
    ...
    >
      <defs>
        <filter id="skeleton-glow" x="-80%" y="-80%" width="260%" height="260%">
          <feGaussianBlur in="SourceGraphic" stdDeviation="0.9" result="blurWide" />
          <feGaussianBlur in="SourceGraphic" stdDeviation="0.35" result="blurTight" />
          <feMerge>
      ...
      {line(pt(LM.NOSE), midShoulder, headColor, 0.8)}
      {dot(pt(LM.NOSE), headColor, 1.3)}
      {label(pt(LM.NOSE), "headTiltDeg", headOk, 3, -3)}

      {/* Front arm — colored + labeled by frontElbowDeg */}
      {line(pt(front.shoulder), pt(front.elbow), elbowColor, 0.8)}
      {line(pt(front.elbow), pt(front.wrist), elbowColor, 0.8)}
      {dot(pt(front.elbow), elbowColor)}
      {dot(pt(front.wrist), elbowColor)}
      {label(pt(front.elbow), "frontElbowDeg", elbowOk, 3, 1)}

      {/* Front leg — colored + labeled by frontKneeDeg */}
      {line(pt(front.hip), pt(front.knee), kneeColor, 0.8)}
      {line(pt(front.knee), pt(front.ankle), kneeColor, 0.8)}
      {dot(pt(front.knee), kneeColor)}
      {dot(pt(front.ankle), kneeColor)}
