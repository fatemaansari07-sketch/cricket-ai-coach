...
/**
 * FIX: "skeleton locks onto the bowler instead of the batsman" bug.
 *
 * pickTrackedPerson()'s old "no lock yet" rule picked whoever was most
 * centered+largest in the very FIRST frame. In a real batting clip that's
 * often the BOWLER — they're mid run-up, closer to the camera, and the
 * batsman is smaller/waiting further away — so the very first decision
 * was already wrong, and everything after just tracked that mistake
 * faithfully.
 *
 * Fix: don't decide from one frame. Watch everyone in frame for the first
 * ~1.5 seconds, track each person as their own path (nearest-centroid
 * matching frame to frame), then choose based on real cricket behaviour:
 *   - batting/fielding: the batsman/fielder is relatively STILL while
 *     anyone else (a bowler running in) moves a lot -> pick whoever moved
 *     the LEAST.
 *   - bowling: the bowler is the one RUNNING -> pick whoever moved the MOST.
 * Only after that decision does normal frame-to-frame lock tracking begin.
 */
function createDeferredPersonTracker(category = "batting", bufferSeconds = 1.5) {
  let decided = false;
  let lockedCentroid = null;
  let bufferStartT = null;
  const buffered = []; // { t, scored }
  let tracks = []; // { points: [{ t, centroid, size, landmarks }] }
  ...
  const assignToTracks = (t, scored) => {
    ...
    for (const track of tracks) {
      ...
      scored.forEach((c, i) => {
        ...
        const d = Math.hypot(c.centroid.x - lastPt.centroid.x, c.centroid.y - lastPt.centroid.y);
        if (d < bestDist) { bestDist = d; bestI = i; }
      });
      if (bestI >= 0 && bestDist < 0.22) { track.points.push({ t, ...scored[bestI] }); used.add(bestI); }
    }
    scored.forEach((c, i) => { if (!used.has(i)) tracks.push({ points: [{ t, ...c }] }); });
  };

  const decideTarget = () => {
    const minPresence = Math.max(2, Math.ceil(buffered.length * 0.4));
    const viable = tracks.filter((tr) => tr.points.length >= minPresence);
    const pool = viable.length ? viable : tracks;
    const withMovement = pool.map((tr) => {
      let total = 0, n = 0;
      for (let i = 1; i < tr.points.length; i++) {
        total += Math.hypot(tr.points[i].centroid.x - tr.points[i - 1].centroid.x, tr.points[i].centroid.y - tr.points[i - 1].centroid.y);
        n++;
      }
      return { track: tr, movement: n ? total / n : 0 };
    });
    if (!withMovement.length) return null;
    return category === "bowling"
      ? withMovement.reduce((a, b) => (b.movement > a.movement ? b : a)).track
      : withMovement.reduce((a, b) => (b.movement < a.movement ? b : a)).track;
  };

  return {
    // Returns { pick } for this tick's chosen person, or { replay: [{t, landmarks, centroid}] }
    // the very moment the buffered decision resolves (caller should back-fill those ticks).
    consider(candidateLandmarksList, t) {
      const scored = scoreCandidates(candidateLandmarksList);
      if (!scored.length) return { pick: null };
      ...
      if (decided) {
        ...
        lockedCentroid = best.centroid;
        return { pick: best };
      }

      if (bufferStartT === null) bufferStartT = t;
      assignToTracks(t, scored);
      buffered.push({ t, scored });

      if (t - bufferStartT >= bufferSeconds) {
        const target = decideTarget();
        decided = true;
        if (!target) return { pick: null };
        const byT = new Map(target.points.map((p) => [p.t, p]));
        const replay = buffered.map((b) => byT.get(b.t)).filter(Boolean);
        lockedCentroid = replay.length ? replay[replay.length - 1].centroid : null;
        return { replay };
      }

      return { pending: true }; // still buffering — nothing to output yet for this tick
    },

    // Safety valve for short clips / sparse sampling where the buffer
    // window never naturally elapses — decide with whatever we have so
    // buffered ticks aren't silently dropped.
    forceDecide() {
      if (decided || !buffered.length) return { replay: [] };
      const target = decideTarget();
      decided = true;
      if (!target) return { replay: [] };
      const byT = new Map(target.points.map((p) => [p.t, p]));
      const replay = buffered.map((b) => byT.get(b.t)).filter(Boolean);
      lockedCentroid = replay.length ? replay[replay.length - 1].centroid : null;
      return { replay };
 ...
 */
export async function extractPoseSequenceFromVideo(videoFile, numFrames = 20, hintTimestamps = null, category = "batting") {
  const landmarker = await getLandmarker();
  ...
  try {
    ...
    let ctx = null;
    const frames = [];
    const ballByT = new Map(); // ball detections keyed by exact tick time, since person-lock decisions are deferred
    let previousBall = null;
    const personTracker = createDeferredPersonTracker(category);
      ...
      await new Promise((resolve, reject) => {
        ...
        const onFrame = (_now, metadata) => {
          ...
          if (t >= duration * 0.08 && t <= duration * 0.95) {
            ...
            if (ball) { previousBall = ball; ballByT.set(t, ball); }

            if (result.landmarks && result.landmarks.length > 0) {
              const outcome = personTracker.consider(result.landmarks, t);
              if (outcome.replay) {
                // Buffered decision just resolved — back-fill every ticked
                // frame we deferred, now that we know who "our player" is.
                for (const r of outcome.replay) frames.push({ t: r.t, landmarks: r.landmarks, ball: ballByT.get(r.t) || null });
              } else if (outcome.pick) {
                frames.push({ t, landmarks: outcome.pick.landmarks, ball });
              }
              // outcome.pending -> still buffering, nothing pushed yet for this tick.
            }
          }

          if (video.ended || t >= duration * 0.95) {
            // Never got a full 1.5s buffer window (very short clip) —
            // decide with what we have so buffered ticks aren't lost.
            const forced = personTracker.forceDecide();
            for (const r of forced.replay || []) frames.push({ t: r.t, landmarks: r.landmarks, ball: ballByT.get(r.t) || null });
