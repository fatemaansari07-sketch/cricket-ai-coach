...
function drawBall(ctx, ball, trail, w, h) {
  ...
  ctx.save();
  if (trail?.length > 1) {
    // Bold, glowing, smooth red trajectory arc — release -> bounce -> end —
    // drawn with a quadratic curve through the tracked points rather than a
    // thin dashed polyline, so it reads clearly even on a small phone screen.
    ctx.strokeStyle = "#ef4444"; ctx.lineWidth = Math.max(4, w / 130); ctx.lineCap = "round"; ctx.lineJoin = "round";
    ctx.shadowColor = "rgba(239,68,68,.9)"; ctx.shadowBlur = 10;
    ctx.beginPath();
    ctx.moveTo(trail[0].x * w, trail[0].y * h);
    for (let i = 1; i < trail.length - 1; i++) {
      const midX = ((trail[i].x + trail[i + 1].x) / 2) * w;
      const midY = ((trail[i].y + trail[i + 1].y) / 2) * h;
      ctx.quadraticCurveTo(trail[i].x * w, trail[i].y * h, midX, midY);
    }
    ctx.lineTo(trail[trail.length - 1].x * w, trail[trail.length - 1].y * h);
    ctx.stroke();
    ctx.shadowBlur = 0;

    const bounce = trail.findIndex((p,i)=>i>1 && trail[i-1].y < p.y && trail[i+1]?.y < p.y);
    const markers = [{p:trail[0],label:"RELEASE"}];
    if (bounce > 0) markers.push({p:trail[bounce],label:"BOUNCE / TAPPA"});
    markers.push({p:trail[trail.length-1],label:"END"});
    ctx.font=`800 ${Math.max(9,w/65)}px sans-serif`;
    markers.forEach(({p,label})=>{
      const bw = label.length * (w/95) + 14;
      ctx.fillStyle="rgba(5,7,11,.85)";ctx.fillRect(p.x*w+7,p.y*h-15,bw,20);
