...
/* "AI umpire" read of the delivery — line/length/swing computed from the
   REAL tracked trajectory, not invented numbers. A single 2D phone camera
   with no stump/pitch reference can't honestly measure "5.3° spin" or an
   exact mph — what it CAN measure from pixel positions is where the ball
   bounced relative to the batsman, how far up the pitch, and how much it
   drifted sideways. That's reported as categories a player can act on,
   not manufactured precision. */
export function analyzeBallLine(trajectory,batsmanCentroidX,batsmanFeetY){
  const pts=trajectory?.points; if(!pts||pts.length<4)return null;
  const bounceIdx=trajectory.bounceIndex; const bouncePoint=trajectory.bouncePoint;
  const refX=typeof batsmanCentroidX==='number'?batsmanCentroidX:0.5;
  const lineRef=bouncePoint||pts.at(-1);
  const lineOffset=lineRef.x-refX;
  const line=Math.abs(lineOffset)<0.05?'Seedha (stumps ki line)':(lineOffset<0?'Off-side ki line':'Leg-side ki line');
  let length='Good length';
  if(bouncePoint&&typeof batsmanFeetY==='number'){
    const gap=batsmanFeetY-bouncePoint.y;
    length=gap<0.12?'Full length':gap>0.32?'Short pitched':'Good length';
  }
  const first=pts[0],last=pts.at(-1);
  const straightX=(frac)=>first.x+(last.x-first.x)*frac;
  let maxDev=0; pts.forEach((p,i)=>{const frac=i/(pts.length-1); maxDev=Math.max(maxDev,Math.abs(p.x-straightX(frac)));});
  const swing=maxDev<0.02?'Seedha aayi, koi swing nahi':maxDev<0.05?'Thoda movement':'Achha khasa swing/turn';
  return {line,length,swing,bouncePoint,bounceIndex:bounceIdx,deviationFrac:Math.round(maxDev*1000)/1000};
}
