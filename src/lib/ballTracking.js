...
/**
 * Pro-tier (₹499 plan) AI ball trajectory detection. Samples `count` frames
 * evenly across the video, sends them to /api/detect-ball-trajectory (which
 * calls a Replicate open-vocabulary object detector per frame), and returns
 * a smoothed release -> bounce -> end trajectory.
 *
 * This is strictly additive: on any failure (no server function deployed,
 * no REPLICATE_API_TOKEN, network error, no confident detections) it
 * returns null so the caller falls back to the local per-frame heuristic
 * (detectBallCandidate) that already runs during pose extraction.
 */
export async function detectTrajectoryViaAI(file, { count = 10, startT = 0, endT } = {}) {
  try {
    const { grabFrame } = await import("./poseEstimation");
    ...
    const duration = endT ?? video.duration;
    URL.revokeObjectURL(url);

    const step = Math.max(0.05, (duration - startT) / (count - 1));
    const timestamps = Array.from({ length: count }, (_, i) => startT + i * step);

    const frames = [];
    for (const t of timestamps) {
      const canvas = await grabFrame(file, t);
      // Downscale before encoding — keeps the request payload small and the
      // detector doesn't need full resolution to find a ball-sized object.
      const small = document.createElement("canvas");
      const scale = Math.min(1, 480 / canvas.width);
      small.width = Math.round(canvas.width * scale);
      small.height = Math.round(canvas.height * scale);
      small.getContext("2d").drawImage(canvas, 0, 0, small.width, small.height);
      frames.push({ dataUrl: small.toDataURL("image/jpeg", 0.7), t });
    }

    const res = await fetch("/api/detect-ball-trajectory", {
      method: "POST",
    ...
    if (!points?.length || points.length < 3) return null; // not enough to draw a meaningful arc
    return points;
  } catch {
    return null;
  }
