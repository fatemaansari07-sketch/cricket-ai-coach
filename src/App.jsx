...
import { Home, Video, Calendar, Store, ArrowLeftRight, LogOut, Crown, ShieldCheck, School, TrendingUp, Gauge, Sparkles } from "lucide-react";
...
import SpeedGunScreen from "./screens/SpeedGunScreen";
import CoachChatScreen from "./screens/CoachChatScreen";

const BASE_NAV = [
  { id: "home", icon: Home, label: "Home" },
  { id: "analyze", icon: Video, label: "Analyze" },
  { id: "speed", icon: Gauge, label: "Speed Gun" },
...
export default function App() {
  ...
  const NAV = [
    ...
    ...(planTier === "special_999" ? [{ id: "coach", icon: Sparkles, label: "Coach" }] : []),
      ...
      <div className="w-full max-w-md flex flex-col" style={{ background: C.app, height: "100vh" }}>
        ...
        <div className="flex items-center justify-between px-4 py-4 shrink-0" style={{ borderBottom: `1px solid ${C.border}` }}>
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <Crown size={11} /> {planTier === "free" ? "FREE" : planTier === "pro_99" ? "PRO ₹99" : planTier === "video_499" ? "VIDEO ₹499" : "COACH ₹999"}
            </button>
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4">
          ...
          {tab === "coach" && <CoachChatScreen planTier={planTier} />}
        </div>

        {/* Bottom nav */}
        <div className="flex items-stretch py-2 px-1.5 shrink-0" style={{ background: C.gold }}>
          {NAV.map((n) => {
            const Icon = n.icon;
            ...
            return (
              <button
                key={n.id}
                onClick={() => setTab(n.id)}
                className="flex-1 min-w-0 flex flex-col items-center justify-center gap-0.5 py-1.5 mx-0.5 rounded-xl active:scale-90 transition-transform"
                style={{ background: active ? "#0B0F17" : "transparent" }}
              >
                <Icon size={16} strokeWidth={2.5} style={{ color: active ? C.gold : "#0B0F17" }} />
                <span
                  className="text-[8.5px] font-bold tracking-wide whitespace-nowrap overflow-hidden text-ellipsis max-w-full px-0.5"
                  style={{ color: active ? C.gold : "#0B0F17" }}
                >
                  {n.label}
                </span>
              </button>
