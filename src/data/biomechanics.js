...
export function evaluatePose(landmarks, category, handedness = "right") {
  ...
  // Getting this backwards was silently checking the wrong arm/leg entirely.
  ...
  const frontElbow = Math.round(angleBetween(front.shoulder, front.elbow, front.wrist) ?? 0);
  const frontKnee = Math.round(angleBetween(front.hip, front.knee, front.ankle) ?? 0);

  const angles = { headTiltDeg: headTilt, frontElbowDeg: frontElbow, frontKneeDeg: frontKnee };

  // Confidence check: if MediaPipe itself wasn't sure it could see these
  // points clearly (bad angle, occlusion, poor lighting), don't present the
  // numbers as if they were reliable — flag it instead of guessing silently.
