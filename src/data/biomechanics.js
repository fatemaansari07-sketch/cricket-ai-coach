...
export function evaluatePoseSequence(frames, category, handedness = "right") {
  ...
  // Real phase detection from hand-speed, not frame position — see
  // detectShotPhases() for why. This is what fixes "stance" showing an
  // already-mid-shot pose: we now know exactly which frame is the quiet
  // moment before the swing, which one is nearest actual bat-ball
  // contact, and which one is after the body has settled again.
  const phases = frames.length >= 5 ? detectShotPhases(frames, handedness) : null;
