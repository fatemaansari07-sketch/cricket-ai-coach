...
import { detectMovementPhases } from "../lib/phaseDetection";
...
export function computeVideoFrameEvaluation(landmarks, category = "batting", handedness = "right", rulesOverride = null) {
  if (!landmarks) return null;
  const rules = rulesOverride || IDEAL_RANGES[category];
  const { angles, avgVisibility } = computeFrameAngles(landmarks, handedness);
  const scored = scoreAngles(angles, rules, category, "frame");
  const jointStatus = Object.fromEntries(Object.keys(rules).map((k) => [k, angles[k] >= rules[k].min && angles[k] <= rules[k].max]));
  const jointDetail = Object.fromEntries(Object.entries(rules).map(([k, r]) => [k, { value: Math.round(angles[k]), min: r.min, max: r.max }]));
  const firstIssue = scored.issues.slice().sort((a,b) => b.severity-a.severity)[0];
  return { angles, avgVisibility, score: scored.score, jointStatus, jointDetail, issues: scored.issues, message: firstIssue?.message || "Position theek hai", phase: "MOVEMENT" };
}
...
export function evaluatePoseSequence(frames, category, handedness = "right") {
  ...
  // Pick phases from actual movement, not fixed percentages.
  const detectedPhases = detectMovementPhases(frames, category);
  const midFrameIndex = detectedPhases.impactIndex;
  ...
  if (perFrame.length >= 4) {
    ...
    const stanceIndices = [detectedPhases.stanceIndex, Math.min(detectedPhases.stanceIndex + 1, perFrame.length - 1)];
    const stanceFrames = stanceIndices.map(i => perFrame[i]).filter(Boolean);
    const stanceAngles = {
    ...
    stanceSkeleton = {
      landmarks: frames[detectedPhases.stanceIndex]?.landmarks || null,
      atSeconds: frames[detectedPhases.stanceIndex]?.t ?? null,
    ...
    const ftIndices = [detectedPhases.followThroughIndex, Math.min(detectedPhases.followThroughIndex + 1, perFrame.length - 1)];
    const ftFrames = ftIndices.map(i => perFrame[i]).filter(Boolean);
    const ftAngles = {
    ...
    const lastFrameIndex = detectedPhases.followThroughIndex;
    followThroughSkeleton = {
      landmarks: frames[lastFrameIndex]?.landmarks || null,
      atSeconds: frames[lastFrameIndex]?.t ?? null,
  ...
  return {
    ...
    phaseDetection: { ...detectedPhases, motion: undefined },
    skeletonLandmarks,
    skeletonAtSeconds,
    jointStatus,
    jointDetail,
