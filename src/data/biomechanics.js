import { LM, detectShotPhases } from "../lib/poseEstimation";
...
/** How far outside [min,max] a value is, normalized to that rule's own
 *  range width — so a joint with a narrow ideal window isn't automatically
 *  scored "more severe" than one with a wide window just because of units.
 *  This is what the Priority Engine uses to rank problems against each other. */
function severityFor(value, rule) {
  const width = Math.max(1, rule.max - rule.min);
  if (value >= rule.min && value <= rule.max) return 0;
  const dist = value < rule.min ? rule.min - value : value - rule.max;
  return Math.max(0.05, Math.min(1, dist / width));
}

function scoreAngles(angles, rules, category, phase = "mid") {
  const correct = [];
  const incorrect = [];
  const issues = [];
  let passCount = 0;
  Object.entries(rules).forEach(([key, rule]) => {
    ...
    if (ok) {
      ...
      incorrect.push(line);
      // baseKey (e.g. "frontElbowDeg") identifies the underlying joint problem
      // across stance/mid-shot/follow-through and across sessions, so the
      // Priority Engine + retest logic can track "the same issue" over time.
      issues.push({
        key: `${phase}.${key}`,
        baseKey: key,
        label: rule.label,
        category,
        phase,
        severity: severityFor(value, rule),
        message: rule.tip(value, ok),
      });
    }
  });
  const score = Math.round((passCount / Object.keys(rules).length) * 100);
  return { correct, incorrect, score, issues };
}
...
export function evaluatePose(landmarks, category, handedness = "right") {
  ...
  const { correct, incorrect, score, issues } = scoreAngles(angles, rules, category, "mid");
  return { category, score, correct, incorrect, issues, angles, confidence, handedness };
}
 ...
 */
export function evaluatePoseSequence(frames, category, handedness = "right", geminiPhaseTimestamps = null) {
  const rules = IDEAL_RANGES[category];

  if (!frames || frames.length === 0) {
    return { category, score: 0, correct: [], incorrect: [], issues: [], angles: null, confidence: "low", handedness, framesUsed: 0 };
  }

  const perFrame = frames.map((f) => computeFrameAngles(f.landmarks, handedness));

  // Real phase detection: prefer Gemini's video-level read of the shot
  // (it can watch the whole clip and actually tell "this is bat-ball
  // contact" the way a human would) over our own motion-speed guess.
  // MediaPipe never loses its job here — it still does 100% of the
  // actual ANGLE measurement; Gemini only ever points at WHICH frame to
  // measure. If Gemini's call failed or wasn't available, we fall back
  // to the motion-based detector so nothing breaks.
  const nearestFrameIndex = (seconds) => {
    if (seconds == null || !frames.length) return null;
    let best = 0, bestDiff = Infinity;
    frames.forEach((f, i) => {
      const diff = Math.abs(f.t - seconds);
  ...
  let phases = null;
  if (geminiPhaseTimestamps) {
    const stanceIdx = nearestFrameIndex(geminiPhaseTimestamps.stanceSec);
    const contactIdx = nearestFrameIndex(geminiPhaseTimestamps.contactSec);
    const followThroughIdx = nearestFrameIndex(geminiPhaseTimestamps.followThroughSec);
    if (stanceIdx !== null && contactIdx !== null && followThroughIdx !== null) {
      phases = { stanceIdx, contactIdx, followThroughIdx, method: "gemini" };
    }
  }
  if (!phases && frames.length >= 5) {
    phases = detectShotPhases(frames, handedness);
  }

  // A small window (the detected frame + 1 neighbour either side, clamped
  // to the clip) instead of a single frame — keeps the median-based noise
  // resistance the previous version had, but centered on the RIGHT moment
  // instead of the whole clip or a fixed position.
  const windowAround = (idx, spread = 1) => {
    const start = Math.max(0, idx - spread);
    const end = Math.min(perFrame.length - 1, idx + spread);
    return perFrame.slice(start, end + 1);
  };

  // Mid-shot / "contact" reading now comes from frames right around the
  // detected peak hand-speed moment (approx. bat-ball contact), not a
  // median across the ENTIRE clip (which used to blend stance + swing +
  // follow-through into one mushy number).
  const contactFrames = phases ? windowAround(phases.contactIdx, 1) : perFrame;
  const angles = {
    headTiltDeg: median(contactFrames.map((f) => f.angles.headTiltDeg)),
    frontElbowDeg: median(contactFrames.map((f) => f.angles.frontElbowDeg)),
    frontKneeDeg: median(contactFrames.map((f) => f.angles.frontKneeDeg)),
  };
  ...
  // Middle frame = the single most representative pose for drawing the
  // skeleton overlay. The scored angles above are a median across all
  // frames (more accurate), but for a visual overlay one clean frame is
  // what a person expects to see — the joint colors still reflect the
  // real median-based pass/fail, not just this one frame's numbers.
  const midFrameIndex = phases ? phases.contactIdx : Math.floor(frames.length / 2);
  const skeletonLandmarks = frames[midFrameIndex]?.landmarks || null;
  const skeletonAtSeconds = frames[midFrameIndex]?.t ?? null;
  const midIssueKeys = new Set(issues.map((i) => i.baseKey));
  const jointStatus = Object.fromEntries(
    Object.keys(rules).map((key) => [key, !midIssueKeys.has(key)])
  );
  const jointDetail = Object.fromEntries(
    Object.entries(rules).map(([key, rule]) => [key, { value: Math.round(angles[key]), min: rule.min, max: rule.max }])
  );

  // Stance = the actual quiet frame just before the swing starts (from
  // detectShotPhases), Follow-through = the frame after impact where the
  // body has settled again — not "first 2 / last 2 frames of the clip".
  let stance = null;
  let followThrough = null;
  let stanceSkeleton = null;
  let followThroughSkeleton = null;
  if (phases && perFrame.length >= 5) {
    const stanceRules = STANCE_RANGES[category];
    const stanceFrames = windowAround(phases.stanceIdx, 1);
    const stanceAngles = {
      headTiltDeg: median(stanceFrames.map((f) => f.angles.headTiltDeg)),
      frontKneeDeg: median(stanceFrames.map((f) => f.angles.frontKneeDeg)),
    };
    stance = scoreAngles(stanceAngles, stanceRules, category, "stance");
    const stanceIssueKeys = new Set(stance.issues.map((i) => i.baseKey));
    stanceSkeleton = {
      landmarks: frames[phases.stanceIdx]?.landmarks || null,
      atSeconds: frames[phases.stanceIdx]?.t ?? null,
      jointStatus: Object.fromEntries(Object.keys(stanceRules).map((k) => [k, !stanceIssueKeys.has(k)])),
      jointDetail: Object.fromEntries(Object.entries(stanceRules).map(([k, r]) => [k, { value: Math.round(stanceAngles[k]), min: r.min, max: r.max }])),
    };

    const ftRules = FOLLOW_THROUGH_RANGES[category];
    const ftFrames = windowAround(phases.followThroughIdx, 1);
    const ftAngles = {
      headTiltDeg: median(ftFrames.map((f) => f.angles.headTiltDeg)),
      frontElbowDeg: median(ftFrames.map((f) => f.angles.frontElbowDeg)),
    };
    followThrough = scoreAngles(ftAngles, ftRules, category, "followThrough");
    const ftIssueKeys = new Set(followThrough.issues.map((i) => i.baseKey));
    followThroughSkeleton = {
      landmarks: frames[phases.followThroughIdx]?.landmarks || null,
      atSeconds: frames[phases.followThroughIdx]?.t ?? null,
      jointStatus: Object.fromEntries(Object.keys(ftRules).map((k) => [k, !ftIssueKeys.has(k)])),
      jointDetail: Object.fromEntries(Object.entries(ftRules).map(([k, r]) => [k, { value: Math.round(ftAngles[k]), min: r.min, max: r.max }])),
    };
  }
  ...
  // Most severe problem first — this is what the Priority Engine reads.
  const allIssues = [...(stance?.issues || []), ...issues, ...(followThrough?.issues || [])]
  ...
  return {
    ...
    phaseTimestamps: phases ? {
      stanceAtSeconds: frames[phases.stanceIdx]?.t ?? null,
      contactAtSeconds: frames[phases.contactIdx]?.t ?? null,
      followThroughAtSeconds: frames[phases.followThroughIdx]?.t ?? null,
      detectionMethod: phases.method,
    } : null,
    skeletonLandmarks,
    skeletonAtSeconds,
    jointStatus,
