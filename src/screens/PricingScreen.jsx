...
import { getVideoCreditStatus } from "../lib/videoCredits";
...
const TIERS = [
  ...
  { id:"pro", name:"AI Coach Video", price:"₹499/mo", badge:"FLAGSHIP", icon:Video, tagline:"Professional shareable coaching video", points:[
    "₹99 plan ka sab kuch", "Ball trajectory + release + bounce + keeper tracking", "Bowler + batsman movement analysis", "Professional annotated coaching video", "Skeleton + angles + correct/wrong overlays", "Share-ready branded video", "3 coaching-video credits/month"
  ]},
];
...
export default function PricingScreen({ planTier, refreshProfile }) {
  ...
  return <div className="space-y-4">
    ...
    {TIERS.map(t=>{const Icon=t.icon;return <GlassCard key={t.id} className="p-4" style={{border:`1px solid ${t.featured?C.gold:planTier===t.id?C.green:C.border}`}}>
      ...
      {t.id !== "basic" && <div className="text-[10px] mb-2" style={{color:C.gold}}>Video credits: {getVideoCreditStatus(t.id).remaining}/{getVideoCreditStatus(t.id).limit} monthly</div>}
