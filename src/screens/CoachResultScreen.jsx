...
export default function CoachResultScreen({
  ...
  const [videoCreditsOverride, setVideoCreditsOverride] = useState(null);
  ...
  return (
    <div className="space-y-4 pt-2">
      ...
      {retest?.outcome === "not_improved" && planTier === "pro_99" && (
        <button
          onClick={onUpgrade}
      ...
      {["video_499","special_999"].includes(planTier) && (() => {
        ...
        return <GlassCard className="p-4">
          ...
          <div className="text-[10px] mt-2" style={{ color: C.gold }}>Video credits: {videoCreditsOverride ?? cs.remaining}/{cs.limit}</div>
          {cs.remaining <= 0 && <button onClick={async()=>{const {data}=await supabase.rpc("recharge_video_credits",{p_amount:100});if(data?.allowed){setVideoCreditsOverride(Number(data.remaining));onCreditsChanged?.()}}} className="w-full mt-3 rounded-xl py-2 text-xs font-bold" style={{background:"rgba(245,158,11,.12)",border:`1px solid ${C.gold}44`,color:C.gold}}>Recharge ₹120 → 100 video credits</button>}
          <div className="flex gap-2 mt-3">
            <SolidButton tone="gold" disabled={generatingVideo || (videoCreditsOverride ?? cs.remaining) <= 0} onClick={async()=>{
      ...
      {planTier === "free" && (
        <GlassCard className="p-4" style={{border:`1px solid ${C.gold}44`}}>
          <div className="text-sm font-extrabold" style={{color:C.text}}>Better result ke liye Pro try karo ✨</div>
          <div className="text-xs mt-1" style={{color:C.muted}}>Gemini-selected Stance, Impact aur Follow-through ke 3 visual frames + skeleton + angles + right/wrong.</div>
          <SolidButton tone="gold" className="mt-3" onClick={onUpgrade}>Pro ₹99 dekho</SolidButton>
        </GlassCard>
      )}
      {planTier === "pro_99" && (
        <GlassCard className="p-4" style={{border:`1px solid ${C.green}44`}}>
          <div className="text-sm font-extrabold" style={{color:C.text}}>Video Visualization try karo 🎥</div>
          <div className="text-xs mt-1" style={{color:C.muted}}>Ab tumhare paas 3-image coaching hai. ₹499 plan me moving skeleton, ball trajectory, bounce/tappa aur shareable coaching video milega.</div>
          <SolidButton tone="green" className="mt-3" onClick={onUpgrade}>Video Visualization ₹499</SolidButton>
        </GlassCard>
      )}
      {planTier === "video_499" && (
        <GlassCard className="p-4" style={{border:`1px solid ${C.gold}44`}}>
          <div className="text-sm font-extrabold" style={{color:C.text}}>Special Coaching ₹999 👑</div>
          <div className="text-xs mt-1" style={{color:C.muted}}>Personal AI coach, full roadmap, skill progression aur cricket-only Coach Chat.</div>
          <SolidButton tone="gold" className="mt-3" onClick={onUpgrade}>Special Coaching dekho</SolidButton>
        </GlassCard>
      )}
