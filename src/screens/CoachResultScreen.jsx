 ...
 */
export default function CoachResultScreen({ coaching, shotType, onPlanReady, showAd, planTier, onUpgrade, videoThumb, videoThumbAspect }) {
  ...
  return (
    <div className="space-y-4 pt-2">
      ...
      {/* Video hero — a real frame from the uploaded clip, not a stock photo.
          Positioning here is deliberately all inline style, not Tailwind
          classes — this box must render at a fixed, contained height with
          the skeleton pixel-locked on top every time. */}
      {videoThumb ? (
        <div style={{ position: "relative", width: "100%", aspectRatio: videoThumbAspect || "16/10", maxHeight: "70vh", borderRadius: 16, overflow: "hidden", background: "#000" }}>
          <img
            src={videoThumb}
            alt=""
            style={{ position: "absolute", top: 0, left: 0, width: "100%", height: "100%", objectFit: videoThumbAspect ? "contain" : "cover" }}
          />
          {showSkeleton && <SkeletonOverlay landmarks={skeletonLandmarks} jointStatus={jointStatus} jointDetail={jointDetail} handedness={handedness} />}
          <div style={{ position: "absolute", inset: 0, background: "linear-gradient(to top, rgba(5,7,11,0.92) 0%, rgba(5,7,11,0.15) 55%, transparent 100%)" }} />
          <div style={{ position: "absolute", top: 12, right: 12, display: "flex", alignItems: "center", padding: "4px 10px", borderRadius: 8, fontSize: 12, fontWeight: 700, background: "rgba(5,7,11,0.7)", color: score >= 80 ? C.green : score >= 65 ? C.gold : C.red, border: `1px solid ${C.border}` }}>
            {score}/100
          </div>
          {showSkeleton && (
            <div style={{ position: "absolute", top: 12, left: 12, display: "flex", gap: 8, padding: "4px 8px", borderRadius: 8, fontSize: 9, fontWeight: 700, background: "rgba(5,7,11,0.7)" }}>
              <span style={{ display: "flex", alignItems: "center", gap: 4, color: C.green }}><span style={{ width: 6, height: 6, borderRadius: 999, background: C.green }} /> SAHI</span>
              <span style={{ display: "flex", alignItems: "center", gap: 4, color: C.red }}><span style={{ width: 6, height: 6, borderRadius: 999, background: C.red }} /> GALAT</span>
            </div>
          )}
          <div style={{ position: "absolute", bottom: 12, left: 16, right: 16 }}>
            <div style={{ fontSize: 10, textTransform: "uppercase", letterSpacing: "0.1em", fontWeight: 600, color: "#D1D5DB" }}>{shotType || category}</div>
            <div style={{ fontSize: 18, fontWeight: 800, color: C.text, fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
              {category[0].toUpperCase() + category.slice(1)} Analysis
            </div>
            {streak > 1 && (
              <div style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 12, marginTop: 4, fontWeight: 600, color: C.gold }}>
