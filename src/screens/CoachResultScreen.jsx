...
export default function CoachResultScreen({
  ...
  const {
    ...
    stanceSkeleton, followThroughSkeleton,
  } = coaching;
  ...
  const shareReport = async () => {
    ...
    const title = "Cricket AI Coach — My Session";

    // Prefer sharing the actual watermarked video (Instagram Reels/WhatsApp/
    // Shorts all accept a shared file) — fall back to text-only share.
    if (annotatedVideoUrl && navigator.canShare) {
      try {
        const blob = await (await fetch(annotatedVideoUrl)).blob();
        const videoFile = new File([blob], "cricket-ai-coach.webm", { type: blob.type || "video/webm" });
        if (navigator.canShare({ files: [videoFile] })) {
          await navigator.share({ title, text, files: [videoFile] });
          return;
        }
      } catch { /* fall through to text share below */ }
    }
    if (navigator.share) {
      try { await navigator.share({ title, text }); } catch { /* user cancelled, ignore */ }
    } else {
    ...
    <div className="space-y-4 pt-2">
      ...
      {annotatedVideoUrl && (
        <GlassCard className="p-3">
          ...
          <video src={annotatedVideoUrl} controls playsInline className="w-full rounded-xl" style={{ background: "#000" }} />
          <div className="text-[10px] mt-2" style={{ color: C.muted }}>Moving skeleton + ball tracking — browser me locally generated.</div>
        </GlassCard>
      )}
      ...
      {hasAnyThumb ? (
        <div>
          <div style={{ display: "flex", gap: 10, overflowX: "auto", scrollSnapType: "x mandatory", WebkitOverflowScrolling: "touch", paddingBottom: 4 }}>
            ...
            />
            <PhaseFrame
              label="Impact" thumb={videoThumb} aspect={videoThumbAspect}
