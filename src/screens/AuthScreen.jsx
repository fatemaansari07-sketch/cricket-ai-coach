...
export default function AuthScreen() {
  ...
  const [showEmailForm, setShowEmailForm] = useState(false); // Google is primary; email is a secondary, collapsed path
  ...
  return (
    <div className="w-full flex items-center justify-center px-6 app-shell-center" style={{ background: C.bg, minHeight: "100dvh" }}>
      <div className="w-full max-w-sm text-center mx-auto">
        ...
        {/* Primary path — Google. This is the only thing most users need to see. */}
        <button
          onClick={() => signInWithGoogle(academyCode)}
          className="w-full flex items-center justify-center gap-2.5 rounded-2xl py-3.5 mb-4 font-semibold text-sm active:scale-95 transition-transform"
        ...
        {/* Everything else is collapsed behind one quiet link — keeps the
            screen looking premium instead of a wall of forms. */}
        {!showEmailForm ? (
          <button
            ...
            style={{ color: C.muted }}
          >
            Ya phir email/phone se continue karo
          </button>
        ) : (
          <div className="mt-2">
