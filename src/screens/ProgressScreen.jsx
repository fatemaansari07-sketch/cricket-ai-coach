...
export default function ProgressScreen({ planTier }) {
  ...
  const isPro = ["pro_99","video_499","special_999"].includes(planTier);
  ...
  if (!isPro) {
    return (
      ...
      <GlassCard className="p-4" style={{border:`1px solid ${C.gold}33`}}>
        <div className="text-sm font-extrabold" style={{color:C.text}}>🗓️ 30-Day Practice Test</div>
        <div className="text-xs mt-1" style={{color:C.muted}}>Har 7 din test video do. Test ke 3 din pehle aur phir daily reminder UI me dikhaya jayega jab practice plan active hoga.</div>
        <div className="grid grid-cols-3 gap-2 mt-3">{[7,14,21].map(d=><div key={d} className="rounded-xl p-2 text-center" style={{background:"rgba(255,255,255,.03)"}}><div className="text-[9px]" style={{color:C.muted}}>Test</div><div className="text-sm font-extrabold" style={{color:C.gold}}>Day {d}</div></div>)}</div>
      </GlassCard>
        <GlassCard className="p-8 text-center">
          ...
          <div className="text-xs mt-1.5" style={{ color: C.muted }}>₹99 Pro se 30-day program, har 7 din test aur improvement tracking unlock karo.</div>
        </GlassCard>
      </div>
    );
  }
