...
import { evaluatePoseSequence, compareAngles, computeMatchPercentage } from "../data/biomechanics";
...
export default function CompareScreen({ planTier }) {
  ...
  const runCompare = async () => {
    ...
    try {
      ...
      const evalA = evaluatePoseSequence(framesA, category);
      const evalB = evaluatePoseSequence(framesB, category);
      setResultA({ eval: evalA, figure: landmarksToFigureProps(midA) });
      setResultB({ eval: evalB, figure: landmarksToFigureProps(midB) });

      if (canProCompare && user) {
        const breakdown = compareAngles(evalA.angles, evalB.angles);
        const matchPct = computeMatchPercentage(breakdown);
        await supabase.from("comparisons").insert({
          user_id: user.id,
          mode: refLabel.trim() ? "self_vs_pro" : "self_vs_self",
          breakdown,
      ...
      {resultA && resultB && (
        <>
          ...
          {canProCompare && (() => {
            ...
            const color = matchPct >= 80 ? C.green : matchPct >= 60 ? C.gold : C.red;
            return (
              <GlassCard className="p-4 text-center">
                <div className="text-[10px] uppercase tracking-widest font-bold" style={{ color: C.muted }}>Match %</div>
                <div className="text-3xl font-extrabold mt-1" style={{ color }}>{matchPct}%</div>
                <div className="text-[10px] mt-1" style={{ color: C.muted }}>{refLabel || "Reference"} ke saath similarity</div>
              </GlassCard>
            );
          })()}
