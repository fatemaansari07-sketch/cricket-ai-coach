...
import { SHOT_TYPES } from "../data/limits";
...
export default function CompareScreen({ planTier }) {
  const canBasicCompare = true;
  const canProCompare = ["pro_99","video_499","special_999"].includes(planTier);
    ...
    return (
      <div className="space-y-4">
        ...
        <GlassCard className="p-8 text-center">
          ...
          <div className="text-xs mt-1.5 leading-relaxed" style={{ color: C.muted }}>Free me same-shot Old vs New compare. Pro me player/reference compare aur deeper breakdown unlock hota hai.</div>
        </GlassCard>
      </div>
    );
  }
  ...
  const runCompare = async () => {
    ...
    try {
      const [framesA, framesB] = await Promise.all([
        extractPoseSequenceFromVideo(fileA, 20, null, category),
        extractPoseSequenceFromVideo(fileB, 20, null, category),
      ]);
  ...
  return (
    <div className="space-y-4">
      <SectionTitle sub={canProCompare ? "Old vs New ya kisi player/reference ke saath compare karo" : "Free me same shot ka Old vs New comparison karo"}>Compare</SectionTitle>
      ...
      <select value={shotType} onChange={e=>setShotType(e.target.value)} className="w-full rounded-xl px-3.5 py-2.5 text-xs outline-none" style={{background:"rgba(0,0,0,.3)",border:`1px solid ${C.border}`,color:C.text}}>{SHOT_TYPES[category].map(s=><option key={s} value={s}>{s}</option>)}</select>

      <div className="grid grid-cols-2 gap-3">
        <VideoPicker label="Old / Pehla Video" file={fileA} onChange={setFileA} />
        <VideoPicker label={canProCompare ? "Reference / New Video" : "New / Dusra Video"} file={fileB} onChange={setFileB} />
      </div>
