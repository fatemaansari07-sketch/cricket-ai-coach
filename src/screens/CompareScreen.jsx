...
import { Lock, Upload, Play, Sparkles, Tag, AlertCircle } from "lucide-react";
import { extractPoseFromVideo, LM } from "../lib/poseEstimation";
import { evaluatePose, compareAngles } from "../data/biomechanics";
...
export default function CompareScreen({ planTier }) {
  ...
  const runCompare = async () => {
    ...
    setResultB(null);
    try {
      const [lmA, lmB] = await Promise.all([extractPoseFromVideo(fileA), extractPoseFromVideo(fileB)]);
      if (!lmA || !lmB) {
        throw new Error("Kisi ek video me body clearly nahi dikha. Achhi lighting me, poora body frame me rakh ke dubara try karo.");
      }
      setResultA({ eval: evaluatePose(lmA, category), figure: landmarksToFigureProps(lmA) });
      setResultB({ eval: evaluatePose(lmB, category), figure: landmarksToFigureProps(lmB) });
    } catch (err) {
