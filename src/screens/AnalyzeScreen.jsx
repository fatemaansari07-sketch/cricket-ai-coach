...
export default function AnalyzeScreen({ isPaid, onPlanReady, setTab }) {
  ...
  const runGeminiVisual=async(r, storagePath)=>{
    ...
    const st=await phase(k.stance), im=await phase(k.impact), ft=await phase(k.followThrough);
    ...
    const visualFrames=[st,im,ft];
    const visualScore=Math.round((st.eval.score+im.eval.score+ft.eval.score)/3);
    r.score=Math.round((r.score+visualScore)/2);r.phases={stance:st.eval.score,midShot:im.eval.score,followThrough:ft.eval.score};r.visualKeyframes={stance:st,impact:im,followThrough:ft};r.visualGemini=json.analysis;r.modelVersion="gemini-2.5-flash+mediapipe-keyframes";
  ...
  const analyze=async(visualChoice=false)=>{
    if(!file||!user||quotaHit)return;setLoading(true);setError(null);setResult(null);setGeminiVisual(visualChoice);
    try{
