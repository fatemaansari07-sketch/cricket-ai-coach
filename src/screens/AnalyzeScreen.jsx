  ...
  const analyze = async () => {
    ...
    try {
      ...
      if (isProUser && useGemini) {
        // "Pro Deep Scan" — Gemini finds WHEN (stance/contact/follow-through),
        // MediaPipe measures the actual angles at those exact moments.
        setStep("Gemini shot ke exact moments dhoond raha hai...");
        let geminiPhases = null;
        try {
          const { data: sessionData } = await supabase.auth.getSession();
          const token = sessionData?.session?.access_token;
          const res = await fetch("/api/detect-shot-phases", {
            method: "POST",
            headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
            body: JSON.stringify({ storagePath: path, category, shotType, handedness }),
          });
          const json = await res.json();
          if (res.ok) geminiPhases = json;
        } catch {
          // Gemini timing call failed — fall through to motion-based detection below, nothing breaks.
        }

        setStep("MediaPipe angles measure kar raha hai...");
        const frames = await extractPoseSequenceFromVideo(file, 20, geminiPhases);
        ...
        r = evaluatePoseSequence(frames, category, handedness, geminiPhases);
        r.phaseDetectionEngine = geminiPhases ? "gemini+mediapipe" : "mediapipe-only";
        ...
        const { data: inserted } = await supabase.from("analyses").insert({
          ...
          issues: r.issues || [],
          pose_keypoints: { angles: r.angles, phaseTimestamps: r.phaseTimestamps },
          model_version: "pose-rules-v1+gemini-phases",
        }).select().single();
        analysisRow = inserted;
      } else {
        setStep("Poori shot ke 10 frames check kar raha hai...");
        const frames = await extractPoseSequenceFromVideo(file, 20);
        if (!frames || frames.length === 0) {
          throw new Error("Video me poora body clearly nahi dikha. Achhi lighting me, poora body frame me rakh ke dubara try karo.");
        }
