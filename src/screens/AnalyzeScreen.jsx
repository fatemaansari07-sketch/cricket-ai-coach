          ...
          {result.phases && (
            <div className="grid grid-cols-3 gap-2.5">
              {[
                ...
                <div key={p.label} className="rounded-xl p-2.5 text-center" style={{ background: C.cardSolid, border: `1px solid ${C.border}` }}>
                  <div className="text-[9px] uppercase tracking-wide mb-1" style={{ color: C.muted }}>{p.label}</div>
                  <div className="text-sm font-bold" style={{ color: p.value >= 70 ? C.green : C.gold }}>{p.value}</div>
                </div>
              ))}
            </div>
          )}
