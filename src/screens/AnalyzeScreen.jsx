...
import { detectTrajectoryViaAI } from "../lib/ballTracking";
import { spendCredits } from "../lib/proCredits";
import { isShotTypeLocked, levelForShotType, firstUnlockedShotType } from "../lib/mastery";
import { submitWeeklyTest } from "../lib/testSchedule";
...
export default function AnalyzeScreen({ isPaid, onPlanReady, setTab }) {
  ...
  const [isWeeklyTest, setIsWeeklyTest] = useState(false);
  const [confirmModal, setConfirmModal] = useState(null);
  const confirmResolverRef = useRef(null);
  const askConfirm = (title, body) =>
    new Promise((resolve) => { confirmResolverRef.current = resolve; setConfirmModal({ title, body }); });
  const resolveConfirm = (val) => { setConfirmModal(null); confirmResolverRef.current?.(val); confirmResolverRef.current = null; };
  ...
  const changeCategory = (c) => {
    setCategory(c);
    setShotType(isPaid ? firstUnlockedShotType(profile, c, SHOT_TYPES[c]) : SHOT_TYPES[c][0]);
    setResult(null);
  };
  ...
  const analyze = async () => {
    ...
    try {
      ...
      if (isProUser && useGemini) {
        const geminiBalance = profile?.gemini_credits_remaining || 0;
        if (geminiBalance < 10) throw new Error("Gemini credits khatam (10 chahiye). Pricing screen se recharge karo.");
        const ok = await askConfirm(
          "Use 10 Gemini Credits?",
          `3-Keyframe Pro Deep Scan ke liye 10 credits lagenge. Balance: ${geminiBalance} → ${geminiBalance - 10}.`
        );
        if (!ok) { setLoading(false); setStep(""); return; }
        await spendCredits({ userId: user.id, type: "gemini", amount: 10, reason: "3_keyframe_analysis", currentBalance: geminiBalance });
        await refreshProfile();
        ...
        const frames = await extractPoseSequenceFromVideo(file, durationHint);
      ...
      // Free and paid plans can see the same core moving skeleton. The
      // expensive part is only server/Gemini work; this annotated render is
      // generated locally from the already-computed pose frames.
      if (r?.analysisFrames?.length) {
        const credit = consumeVideoCredit(planTier);
        let canRender = credit.allowed;
        let renderMsg = planTier === "pro" ? `AI Coach Video credit use ho raha hai (${credit.remaining} left)...` : `Free movement-video credit use ho raha hai (${credit.remaining} left)...`;

        if (!canRender && planTier === "pro") {
          const videoBalance = profile?.coaching_video_credits_remaining || 0;
          if (videoBalance >= 10) {
            const ok = await askConfirm(
              "Use 10 Video Credits?",
              `Free monthly videos khatam. HD trajectory video ke liye 10 credits lagenge. Balance: ${videoBalance} → ${videoBalance - 10}.`
            );
            if (ok) {
              await spendCredits({ userId: user.id, type: "coaching_video", amount: 10, reason: "video_generation", currentBalance: videoBalance });
              await refreshProfile();
              canRender = true;
              renderMsg = "10 Video Credits use ho rahe hain...";
            }
          }
        }
        ...
        if (canRender) {
          setStep(renderMsg);
          // ₹499-tier feature: try AI-based ball trajectory (Replicate open-
          // vocabulary detector) for a cleaner release -> bounce -> end line
          // than the local per-frame heuristic. Purely additive — on any
          // failure it silently keeps the local heuristic's ball points.
          if (planTier === "pro") {
            setStep("AI red line trajectory bana raha hai...");
            const aiPoints = await detectTrajectoryViaAI(file);
            if (aiPoints?.length) {
              for (const frame of r.analysisFrames) {
                const nearest = aiPoints.reduce((best, p) => (!best || Math.abs(p.t - frame.t) < Math.abs(best.t - frame.t) ? p : best), null);
                if (nearest && Math.abs(nearest.t - frame.t) < 0.3) frame.ball = { x: nearest.x, y: nearest.y, confidence: nearest.confidence };
              }
            }
          }
          try {
            const rendered = await renderAnnotatedVideo(file, r.analysisFrames, { showAngles: true, professional: planTier === "pro" });
            if (rendered) setAnnotatedVideoUrl(rendered);
          } catch {
            // Keep the normal frame cards if the browser cannot record canvas video.
          }
        }
      }

      // Tier 4 weekly mastery test: score the shot against this level's pass
      // threshold, unlock the next level on pass, and reschedule the 7-day test.
      if (isWeeklyTest && isPaid && category === "batting") {
        const level = levelForShotType(shotType);
        if (level) await submitWeeklyTest({ supabase, userId: user.id, level: level.id, score: r.score, videoId: videoRow.id });
        await refreshProfile();
      }
            ...
            <div>
              ...
              >
                {(isFree ? SHOT_TYPES[category].slice(0, 2) : SHOT_TYPES[category]).map((s) => {
                  const locked = isPaid && isShotTypeLocked(profile, category, s);
                  return (
                    <option key={s} value={s} disabled={locked}>
                      {locked ? `🔒 ${s} (Level ${levelForShotType(s)?.id} locked)` : s}
                    </option>
                  );
                })}
              </select>
              ...
              {isPaid && category === "batting" && isShotTypeLocked(profile, category, shotType) && (
                <p className="text-[10px] mt-1.5" style={{ color: C.gold }}>
                  🔒 Ye shot abhi locked hai — pehle Mastery tab me pichhla level pass karo.
                </p>
              )}
            </div>

            {isPaid && category === "batting" && !isShotTypeLocked(profile, category, shotType) && (
              <button onClick={() => setIsWeeklyTest((v) => !v)} className="w-full flex items-center justify-between">
                <div className="text-left">
                  <div className="text-sm font-semibold" style={{ color: C.text }}>Weekly Mastery Test</div>
                  <div className="text-[11px] mt-0.5" style={{ color: C.muted }}>Is video ko level test ke tor pe submit karo</div>
                </div>
                <div className="w-11 h-6 rounded-full flex items-center px-0.5 shrink-0" style={{ background: isWeeklyTest ? C.green : C.border }}>
                  <div className="w-5 h-5 rounded-full bg-white transition-transform" style={{ transform: isWeeklyTest ? "translateX(20px)" : "translateX(0)" }} />
                </div>
              </button>
            )}
