  ...
  const analyze = async () => {
    ...
    try {
      ...
      if (r?.analysisFrames?.length) {
        ...
        if (credit.allowed) {
          setStep(planTier === "pro" ? `AI Coach Video credit use ho raha hai (${credit.remaining} left)...` : `Free movement-video credit use ho raha hai (${credit.remaining} left)...`);
          try {
            const keyMoments = [
              r.stanceSkeleton?.atSeconds != null && {
                atSeconds: r.stanceSkeleton.atSeconds, jointStatus: r.stanceSkeleton.jointStatus,
                jointDetail: r.stanceSkeleton.jointDetail, handedness: r.handedness,
                label: Object.values(r.stanceSkeleton.jointStatus || {}).every(Boolean) ? "SAHI: Stance" : "GALAT: Stance",
              },
              r.skeletonAtSeconds != null && {
                atSeconds: r.skeletonAtSeconds, jointStatus: r.jointStatus, jointDetail: r.jointDetail, handedness: r.handedness,
                label: Object.values(r.jointStatus || {}).every(Boolean) ? "SAHI: Impact" : "GALAT: Impact",
              },
              r.followThroughSkeleton?.atSeconds != null && {
                atSeconds: r.followThroughSkeleton.atSeconds, jointStatus: r.followThroughSkeleton.jointStatus,
                jointDetail: r.followThroughSkeleton.jointDetail, handedness: r.handedness,
                label: Object.values(r.followThroughSkeleton.jointStatus || {}).every(Boolean) ? "SAHI: Follow-Through" : "GALAT: Follow-Through",
              },
            ].filter(Boolean);
            const rendered = await renderAnnotatedVideo(file, r.analysisFrames, { showAngles: true, professional: planTier === "pro", keyMoments });
