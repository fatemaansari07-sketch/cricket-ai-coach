  ...
  const analyze = async () => {
    ...
    try {
      ...
      if (isProUser && useGemini) {
        ...
        const captureAligned = async (atSeconds) => {
          if (atSeconds == null) return null;
          try {
            const c = await captureFrameDataUrl(file, atSeconds);
            return { dataUrl: c.dataUrl, aspect: c.width / c.height };
          } catch {
            return null;
          }
        };

        const midShot = await captureAligned(r.skeletonAtSeconds);
        if (midShot) { setVideoThumb(midShot.dataUrl); setVideoThumbAspect(midShot.aspect); }

        const stanceShot = await captureAligned(r.stanceSkeleton?.atSeconds);
        if (stanceShot) { setStanceThumb(stanceShot.dataUrl); setStanceThumbAspect(stanceShot.aspect); }

        const ftShot = await captureAligned(r.followThroughSkeleton?.atSeconds);
        if (ftShot) { setFollowThroughThumb(ftShot.dataUrl); setFollowThroughThumbAspect(ftShot.aspect); }
